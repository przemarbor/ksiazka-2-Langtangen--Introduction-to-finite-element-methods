...
\begin{itemize}
	...
	\item Element contributions to the global linear system are collected in an element matrix and vector, which must be assembled into the global system using the degree of freedom mapping (\mycode{dof\_map}) or the node numbering mapping (\mycode{elements}), depending on which terminology that is used.
